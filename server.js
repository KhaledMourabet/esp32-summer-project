...
// Find it in the Arduino IDE under Tools → Port.
// Windows: 'COM3', 'COM4', 'COM5' … (check Device Manager if unsure)
// Mac:     '/dev/cu.usbmodem14101'  (varies)
// Linux:   '/dev/ttyACM0'           (or /dev/ttyUSB0)
//
// TIP — auto-detect on Windows: leave this as null and the server
// will scan all COM ports and pick the first Arduino it finds.
// Set it to a string like 'COM5' to pin a specific port.
const SERIAL_PORT_OVERRIDE = 'COM3'; // ← change this (or set null to auto-detect)
...
// serialPortReady tracks whether we successfully opened the Arduino.
// When false, sendToArduino() silently does nothing instead of crashing.
// This lets the web UI work for testing even without the Arduino plugged in.
let serialPortReady = false;
let port = null;

// openSerialPort(portName) opens a specific port and wires up the parser.
function openSerialPort(portName) {
  port = new SerialPort({ path: portName, baudRate: BAUD_RATE });

  // ReadlineParser turns the raw byte stream into one message per line.
  ...
  port.on('error', (err) => {
    ...
    console.error(`Serial port error on ${portName}:`, err.message);
    console.error('Arduino communication disabled. Fix the port and restart.');
  });

  port.on('close', () => {
    serialPortReady = false;
    console.warn(`Serial port ${portName} closed unexpectedly.`);
  });

  // Every complete line from the Arduino is handled here.
  parser.on('data', (line) => {
    const msg = line.trim();
    if (!msg) return;
    console.log(`Arduino → Server: "${msg}"`);

    if      (msg === 'ADMIN')                          handleAdminPress();
    else if (msg === 'SPINDONE')                       handleSpinDone();
    else if (['P1','P2','P3','P4'].includes(msg))      handlePlayerPress(msg);
  });
}

// autoDetectPort() scans all available serial ports and returns the name
// of the first one that looks like an Arduino (matched by manufacturer string
// or by the Arduino Mega's USB VID 0x2341).
// Returns null if nothing is found.
async function autoDetectPort() {
  const ports = await SerialPort.list();
  console.log('Available serial ports:', ports.map(p => p.path).join(', ') || 'none');

  const match = ports.find(p =>
    (p.manufacturer && p.manufacturer.toLowerCase().includes('arduino')) ||
    (p.vendorId && p.vendorId.toLowerCase() === '2341') // Arduino official VID
  );
  return match ? match.path : null;
}

// Initialise serial: use the override if set, otherwise auto-detect.
(async () => {
  let portName = SERIAL_PORT_OVERRIDE;

  if (!portName) {
    console.log('SERIAL_PORT_OVERRIDE is null — scanning for Arduino...');
    portName = await autoDetectPort();
    if (!portName) {
      console.warn('No Arduino found. Running without serial. Plug in the Arduino and restart.');
      return; // leave serialPortReady = false
    }
    console.log(`Auto-detected Arduino on ${portName}`);
  }

  openSerialPort(portName);
})();

// Helper: send a command string to the Arduino.
// Safe to call even when the Arduino isn't connected — it just logs a warning.
function sendToArduino(command) {
  if (!serialPortReady || !port) {
    console.warn(`sendToArduino("${command}") skipped — serial port not ready.`);
    return;
  }
